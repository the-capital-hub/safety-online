...
import { getInvoiceTypeLabel } from "@/constants/invoice";
...
export const downloadSalesReportExcel = (report, filters = {}) => {
        ...
        const orderRowsData = (report?.orders || []).map((order) => [
                ...
                ensureValue(getInvoiceTypeLabel(order.invoiceType)),
        ...
        const filterRows = [
                ...
                ["Invoice type", ensureValue(getInvoiceTypeLabel(filters.invoiceType))],
...
export const downloadSalesReportPdf = (report, filters = {}) => {
        ...
        const filterSummary = [
                `Start date: ${formatDate(filters.startDate)}`,
                `End date: ${formatDate(filters.endDate)}`,
                `Invoice type: ${getInvoiceTypeLabel(filters.invoiceType)}`,
                `Generated: ${formatDateTime(generatedAt)}`,
        ];
        ...
        if ((report?.orders || []).length === 0) {
                ...
                report.orders.forEach((order, index) => {
                        ...
                        const invoiceLine = `Units: ${ensureValue(order.unitCount ?? 0)} • Invoice value: ${formatCurrency(
                                order.invoiceValue
                        )} • Invoice type: ${getInvoiceTypeLabel(order.invoiceType)} • Payment: ${ensureValue(
                                order.paymentMethod === "unknown" ? "Unknown" : order.paymentMethod || "—"
                        )}`;
