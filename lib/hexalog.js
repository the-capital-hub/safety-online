...
/**
 * Get shipping cost estimate and estimated delivery time
 * @param {Object} params - Shipping parameters
 * @param {string} params.pickupPincode - Origin pincode
 * @param {string} params.dropPincode - Destination pincode
...
export async function getShippingEstimate(params) {
	if (!params || typeof params !== "object") {
		throw new Error("Shipping parameters are required");
	}
	...
	for (const field of requiredFields) {
		if (!params[field]) {
			throw new Error(`${field} is required for shipping estimate`);
		}
	}

	const token = await getAccessToken();
	const url = `${HEXALOG_BASE_URL}/data/v2/pricing/estimate?include=tat`;
...
hexalogClient.getShippingEstimate = getShippingEstimate;
