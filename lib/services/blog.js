...
const sanitizeCoverImage = (coverImage = {}) => {
        if (!coverImage || typeof coverImage !== "object") {
                return { url: "", alt: "", publicId: "" };
        }

        return {
                url: coverImage.url?.toString?.().trim?.() || "",
                alt: coverImage.alt?.toString?.().trim?.() || "",
                publicId: coverImage.publicId?.toString?.().trim?.() || "",
        };
};
...
const serializeDocument = (doc) => {
        ...
        if (serialized?.coverImage) {
                serialized.coverImage = {
                        url: serialized.coverImage.url || "",
                        alt: serialized.coverImage.alt || "",
                        publicId: serialized.coverImage.publicId || "",
                };
        } else {
                serialized.coverImage = { url: "", alt: "", publicId: "" };
        }
...
export async function createBlogPost(payload) {
        ...
        const post = await BlogPost.create({
                ...
                excerpt,
                content,
                coverImage: sanitizeCoverImage(coverImage),
                author,
                status,
...
export async function updateBlogPost(postId, payload) {
        ...
        const existingPost = await BlogPost.findById(postId).lean();
        ...
        let sanitizedCoverImage = null;
        if (coverImage !== undefined) {
                sanitizedCoverImage = sanitizeCoverImage(coverImage);
                update.coverImage = sanitizedCoverImage;
        }
        ...
        if (sanitizedCoverImage) {
                const previousPublicId = existingPost?.coverImage?.publicId;
                if (previousPublicId && previousPublicId !== sanitizedCoverImage.publicId) {
...
export async function deleteBlogPost(postId) {
        ...
        if (deleted?.coverImage?.publicId) {
                await removeCloudinaryImage(deleted.coverImage.publicId);
        }
