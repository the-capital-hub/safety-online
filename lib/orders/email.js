...
function extractProducts(order) {
        if (Array.isArray(order?.products) && order.products.length > 0) {
                        return order.products.map((item) => {
                                const rawIds = Array.isArray(item.productIds)
                                        ? item.productIds
                                        : Array.isArray(item.productId?.productIds)
                                        ? item.productId.productIds
                                        : [];
        ...
        return order.subOrders.flatMap((subOrder) => {
                ...
                return subOrder.products.map((product) => {
                        const rawIds = Array.isArray(product.productIds)
                                ? product.productIds
                        ...
                        return {
                                ...
                                productIds,
                                hsnCode: product.hsnCode || product.productId?.hsnCode || "",
                        };
                });
        });
}
...
export function normalizeOrderForEmail(order) {
        ...
        const donationAmount = sanitizeAmount(plainOrder?.donationSupport?.amount, 0);
        const donationSupport = {
                amount: donationAmount,
                cause:
                        plainOrder?.donationSupport?.cause ||
                        "Fibrodysplasia Ossificans Progressiva",
                note:
                        plainOrder?.donationSupport?.note ||
                        "Thank you for supporting patients living with FOP.",
        };
        ...
        return {
                ...
                totalAmount: sanitizeAmount(plainOrder.totalAmount, totals.total + donationAmount),
                gst: sanitizedGst,
                ...
                deliveryAddress:
                        ...
                        {},
                billingInfo: plainOrder.billingInfo || {},
                products,
        };
}
...
function buildOrderSummaryHtml(order) {
        ...
        const gstModeMessage =
                order?.gst?.mode === "cgst_sgst"
                        ? "CGST & SGST applied for Karnataka delivery"
                        : order?.gst?.mode === "igst"
                          ? "IGST applied for this delivery"
                          : "";

        const gstInvoiceHtml =
                billing?.gstInvoiceRequested && billing?.gstNumber
                        ? `
                <h3 style="margin-top:24px;border-bottom:1px solid #e5e7eb;padding-bottom:8px;">GST Invoice Details</h3>
                <p style="margin:0;">GSTIN: ${billing.gstNumber}</p>
                ${billing.gstLegalName ? `<p style="margin:0;">Registered Name: ${billing.gstLegalName}</p>` : ""}
                ${billing.gstTradeName ? `<p style="margin:0;">Trade Name: ${billing.gstTradeName}</p>` : ""}
                ${billing.gstAddress ? `<p style="margin:0;font-size:12px;color:#4b5563;">${billing.gstAddress}</p>` : ""}
        `
                        : "";
                ...
                <table style="width:100%;margin-top:8px;">
                        ...
                        ${
                                order.shippingCost > 0
                                        ? `<tr><td style="text-align:right;padding:4px;">Shipping:</td><td style="text-align:right;padding:4px;">${formatCurrency(order.shippingCost)}</td></tr>`
                                        : ""
                        }
                        ${gstRowsHtml}
                        ...
                        ${
                                order?.donationSupport?.amount > 0
                                        ? `<tr><td style="text-align:right;padding:4px;">Donation (FOP):</td><td style="text-align:right;padding:4px;">${formatCurrency(
                                                  order.donationSupport.amount
                                          )}</td></tr>`
                                        : ""
                        }
                        <tr>
...
function buildKeyValueTable(entries = []) {
        ...
        const rows = entries
                ...
                .map(([label, value]) => {
                        ...
                        return `
                                <tr>
                                        <td style="padding:8px;border:1px solid #e5e7eb;background:#f9fafb;font-weight:500;">${label}</td>
                                        <td style="padding:8px;border:1px solid #e5e7eb;">${displayValue}</td>
                                </tr>`;
                })
                .join("");
...
function buildAddressLines(address = {}) {
        ...
        const lines = [
                address.name,
                [address.street, address.area, address.landmark]
                        .filter(Boolean)
                        .join(", ") || null,
...
export async function sendDonationThankYouEmail({ order, to } = {}) {
        const normalizedOrder = normalizeOrderForEmail(order);
        const donationAmount = Number(normalizedOrder?.donationSupport?.amount || 0);
        ...
        if (!donationAmount || donationAmount <= 0 || recipients.length === 0) {
        ...
        const html = `
                <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px;background:#ffffff;color:#111827;">
                        <h2 style="margin:0 0 12px 0;color:#16a34a;">Thank you for supporting FOP patients</h2>
                        <p style="margin:0 0 12px 0;">Hi ${
                                normalizedOrder.customerName || "there"
                        },</p>
                        <p style="margin:0 0 12px 0;">We have received your donation of <strong>${formattedAmount}</strong> to help people living with Fibrodysplasia Ossificans Progressiva (FOP). Your contribution does not go to any seller and will be directed to patient support efforts.</p>
                        <p style="margin:0 0 12px 0;">We appreciate your kindness and solidarity with those battling rare diseases.</p>
                        <p style="margin:0;color:#4b5563;font-size:12px;">If you have any questions, reply to this email and our team will assist you.</p>
                </div>
        `;

        await sendMail({
                to: recipients,
                subject: "Thank you for your FOP donation",
                html,
                metadata: {
...
export async function sendAdminOrderNotificationEmail({ order, to, cc } = {}) {
        const normalizedOrder = normalizeOrderForEmail(order);
        ...
        if (recipients.length === 0 && companyInfo.adminEmail) {
                recipients.push(companyInfo.adminEmail);
        }
        ...
        const formattedDate = new Date(normalizedOrder.orderDate).toLocaleString("en-IN", {
                dateStyle: "medium",
                timeStyle: "short",
        });

        const orderDetailsTable = buildKeyValueTable([
                ["Order Number", normalizedOrder.orderNumber || normalizedOrder._id || "N/A"],
                ["Order Date", formattedDate],
                ["Payment Method", normalizedOrder.paymentMethod || "N/A"],
                ["Payment Status", normalizedOrder.paymentStatus || "N/A"],
                ["Subtotal", formatCurrency(normalizedOrder.subtotal)],
                ["Shipping", formatCurrency(normalizedOrder.shippingCost)],
                ["Tax", formatCurrency(normalizedOrder.tax)],
                ["Discount", formatCurrency(normalizedOrder.discount)],
                ...(normalizedOrder?.donationSupport?.amount > 0
                        ? [["Donation (FOP)", formatCurrency(normalizedOrder.donationSupport.amount)]]
                        : []),
                ["Grand Total", formatCurrency(normalizedOrder.totalAmount)],
        ]);

        const customerDetailsTable = buildKeyValueTable([
                ["Customer Name", normalizedOrder.customerName || "N/A"],
                ["Customer Email", normalizedOrder.customerEmail || "N/A"],
                ["Customer Mobile", normalizedOrder.customerMobile || normalizedOrder.customerPhone || "N/A"],
        ]);

        const addressLines = buildAddressLines(normalizedOrder.deliveryAddress);
        const addressHtml = addressLines.length
                ? `
                        <div style="margin-top:16px;">
                                <h3 style="margin:0 0 8px 0;font-size:16px;">Delivery Address</h3>
                                <p style="margin:0;color:#4b5563;line-height:1.5;">${addressLines.join("<br/>")}</p>
                        </div>`
                : "";
        ...
        const html = `
                <div style="font-family:Arial,sans-serif;max-width:720px;margin:0 auto;padding:24px;color:#111827;background:#ffffff;">
                        <h2 style="margin-top:0;color:#111827;">New order received</h2>
                        <p style="margin:0 0 16px 0;color:#374151;">A new order has been placed on ${
                                companyInfo.name || "the store"
                        }. Below are the order details for your review.</p>
                        ${orderDetailsTable}
                        ${customerDetailsTable}
                        ${addressHtml}
                        <div style="margin-top:24px;">
                                <h3 style="margin:0 0 8px 0;font-size:16px;">Items in this order</h3>
                                <table style="width:100%;border-collapse:collapse;margin-top:8px;">
                                        <thead>
                                                <tr style="background:#f3f4f6;">
                                                        <th style="padding:8px;border:1px solid #e5e7eb;text-align:left;">Product</th>
                                                        <th style="padding:8px;border:1px solid #e5e7eb;text-align:center;width:80px;">Qty</th>
                                                        <th style="padding:8px;border:1px solid #e5e7eb;text-align:right;width:140px;">Total</th>
                                                </tr>
                                        </thead>
                                        <tbody>
                                                ${productsHtml}
                        ...
                        <p style="margin-top:24px;font-size:12px;color:#6b7280;">This notification was sent automatically to store administrators.</p>
                </div>
        `;
        ...
        await sendMail({
                to: recipients,
                cc,
                subject: `New Order Placed - ${normalizedOrder.orderNumber || normalizedOrder._id || "Order"}`,
                html,
                metadata: {
