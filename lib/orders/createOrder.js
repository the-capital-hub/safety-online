	...
	try {
		...
		const paymentFailureReason =
			...
			null;
                ...
                const donationAmount = normalizeNumber(orderData?.donationSupport?.amount, 0);
		...
		const totals = calculateGstTotals({
			...
			shippingCost: totals.shippingCost,
			discount: totals.discount,
                        totalAmount: totals.total + donationAmount,
                        gst: totals.gst,
                        taxableAmount: totals.taxableAmount,
                        donationSupport: {
                                amount: donationAmount,
                                cause:
                                        orderData?.donationSupport?.cause ||
                                        "Fibrodysplasia Ossificans Progressiva",
                                note:
                                        orderData?.donationSupport?.note ||
                                        "Voluntary donation to support patients living with FOP",
                        },
		...
		if (reserveInventory && stockUpdates.length > 0) {
			...
			await Product.bulkWrite(bulkOps, { session });
                ...
                const sellerEntries = Array.from(productsBySeller.entries());
                const shippingCostTotal = normalizeNumber(orderData.shippingCost);
                const totalSellerSubtotal = sellerEntries.reduce((sum, [, items]) => {
                        const sellerSubtotal = items.reduce(
                                (sellerSum, item) => sellerSum + normalizeNumber(item.totalPrice),
                                0
                        );
                        return sum + sellerSubtotal;
                }, 0);

                for (const [sellerKey, items] of sellerEntries) {
                        const sellerSubtotal = items.reduce(
                                (sellerSum, item) => sellerSum + normalizeNumber(item.totalPrice),
                                0
                        );
                        ...
                        const sellerShippingCost =
                                shippingCostTotal > 0
                                        ? totalSellerSubtotal > 0
                                                ? (sellerSubtotal / totalSellerSubtotal) * shippingCostTotal
			...
			subOrderPayloads.push(subOrderPayload);
                ...
                const createdSubOrders = subOrderPayloads.length
                        ? await SubOrder.create(subOrderPayloads, { session, ordered: true })
                        : [];
