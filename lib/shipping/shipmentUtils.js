...
function formatPickupAddress(company) {
	...
	// Find "head office" or fallback to first available address
	...
	return {
		location_type:
			primaryAddress.tagName === "head office" ? "Office" : "Home" || "Office",
		name:
...
export async function createShipmentPackage(
	...
	try {
		...
		let hexalogShipmentId = null;
		let trackingId = null;
		let courierPartner = null;
		let courierGroup = null;
		...
		try {
			...
			if (hexalogResponse && hexalogResponse.success !== false) {
				hexalogShipmentId = hexalogResponse.shipment_id || hexalogResponse.id;
				trackingId = hexalogResponse.tracking_id || hexalogResponse.awb_number;
				courierPartner =
					hexalogResponse.courier_partner || hexalogResponse.partner;
				courierGroup = hexalogResponse.courier_group || hexalogResponse.group;
		...
		const shipmentPackage = {
			...
			hexalogShipmentId: hexalogShipmentId,
			trackingId: trackingId,
			courierPartner: courierPartner,
			courierGroup: courierGroup,
			status: hexalogShipmentId ? "label_generated" : "pending",
		...
		console.log(`📋 Shipment package created for SubOrder ${subOrder._id}:`, {
			...
			hexalogShipmentId: shipmentPackage.hexalogShipmentId,
		});
