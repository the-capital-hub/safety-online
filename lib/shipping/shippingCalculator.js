...
export function calculateShippingParams(orderItems = [], options = {}) {
	// console.log("Calculating shipping params for items:", orderItems, options);
	...
	const packagingItems = orderItems.map((item) => ({
		...
		// Weight in grams (assume item.weight is in kg)
		weight: item.weight ? item.weight * 1000 : 300,
	...
	console.log("Computed package details:", packageDetails);
...
export async function getAutoShippingEstimate(orderItems, options = {}) {
	try {
		...
		// Return estimate along with used package details
