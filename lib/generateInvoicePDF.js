...
export const generateInvoicePDF = async (order) => {
		...
		yPosition += 15;
                ...
                if (order.products && order.products.length > 0) {
                        order.products.forEach((product) => {
                                ...
                                const resolvedProductIds = Array.isArray(product.productIds)
                                        ? product.productIds
                                        : Array.isArray(product.productId?.productIds)
                                        ? product.productId.productIds
                                        : [];
                                const uniqueProductIds = resolvedProductIds
                                        .map((id) => (typeof id === "string" ? id.trim() : String(id || "")))
                                        .filter((id, index, arr) => id.length > 0 && arr.indexOf(id) === index);

                                let blockBottomY = yPosition + 6;

                                if (uniqueProductIds.length > 0 || product.hsnCode) {
                                        doc.setFontSize(8);
                                        doc.setTextColor(102, 102, 102);
                                        let metaY = yPosition + 4;
                                        ...
                                        if (uniqueProductIds.length > 0) {
                                                ...
                                                idLines.forEach((line) => {
                                                        doc.text(line, 25, metaY);
                                                        metaY += 4;
                                                });
                                        }

                                        if (product.hsnCode) {
                                                doc.text(`HSN: ${product.hsnCode}`, 25, metaY);
                                                metaY += 4;
                                        }

                                        blockBottomY = Math.max(blockBottomY, metaY);
                                        doc.setFontSize(10);
                                        doc.setTextColor(0, 0, 0);
                                }
                                ...
                                let detailBottomY = blockBottomY;
                                if (detailLines.length > 0) {
                                        doc.setFontSize(8);
                                        doc.setTextColor(102, 102, 102);
                                        let detailY = blockBottomY + 2;
                                        detailLines.forEach((detail) => {
                                                const wrapped = doc.splitTextToSize(detail.text, 60);
                                                wrapped.forEach((line) => {
                                                        doc.text(line, detail.x, detailY);
                                                        detailY += 4;
                                                });
                                        });
                                        doc.setFontSize(10);
                                        doc.setTextColor(0, 0, 0);
                                        detailBottomY = Math.max(detailBottomY, detailY);
                                }

                                yPosition = Math.max(blockBottomY, detailBottomY) + 6;
		...
		doc.text("Subtotal:", 150, yPosition);
                ...
                const donationValue = sanitizeNumber(order?.donationSupport?.amount);
                if (donationValue > 0) {
                        yPosition += 8;
                        doc.setTextColor(37, 99, 235); // Blue color
                        doc.text("Donation (FOP):", 150, yPosition);
                        doc.text(`${formatAmount(donationValue)}`, 175, yPosition);
                        doc.setTextColor(0, 0, 0);
                }
