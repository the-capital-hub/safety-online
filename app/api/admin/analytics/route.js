				...
				$concat: [
					...
					"-W",
					{
						$cond: [
							...
							{
								$concat: ["0", { $toString: { $isoWeek: "$order.orderDate" } }],
							},
					...
					topProducts: [
						{ $unwind: "$products" },
						{
							$group: {
								...
								productName: {
									$first: {
										$ifNull: ["$products.productName", "Unknown product"],
									},
								},
								totalUnits: {
									$sum: {
		...
		const growth = {
			...
			units:
				previous.totalUnits > 0
					? ((summary.totalUnits - previous.totalUnits) / previous.totalUnits) *
					  100
